...
class UserInput(BaseModel):
    ...
    message: str = Field(
        ...
        examples=["Find albums released by artists who have more than 5 albums"],
    )    
    thread_id: str | None = Field(
        description="Thread ID to persist and continue a multi-turn conversation.",
        default=None,
        examples=["847c6285-8fc9-4560-a83f-4e6285809260"],
    )
        ...
        examples=[
            {
                "message": {
                    "type": "ai",
                    "data": {"content": "Final answer from agent", "type": "ai"},
                }
            }
        ],
    )
...
class ChatMessage(BaseModel):
    ...
    def from_langchain(cls, message: BaseMessage) -> "ChatMessage":
        ...
        original = message_to_dict(message)  # return {"type": message.type, "data": message.model_dump()}
        additional_kwargs = getattr(message, "additional_kwargs", {})

        match message:
            case HumanMessage():
                human_message = cls(
                    type="human", content=message.content, original=original, additional_kwargs=additional_kwargs
                )
                return human_message
            case AIMessage():
                ai_message = cls(
                    type="ai", content=message.content, original=original, additional_kwargs=additional_kwargs
                )
    ...
    def to_langchain(self) -> BaseMessage:
        ...
        match self.type:
            ...
            case "ai":
                return AIMessage(content=self.content, additional_kwargs=self.additional_kwargs)
            case "tool":
                return ToolMessage(content=self.content, tool_call_id=self.tool_call_id, additional_kwargs=self.additional_kwargs)
            case _:
